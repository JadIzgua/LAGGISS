...
def k_fold(data_left, data_right, data_label, k, model_function, model_function_args, folder_path, epochs,
    ...
    for i in range(k):
     ...
     if i>3:
        ...
        checkpoint_path = '/content/drive/MyDrive/LAGGISS/Comp_model_kfold_ES/checkpoints_fold#{}'.format(i+1)
        es = EarlyStopping(monitor='val_accuracy', verbose=1, mode='max',patience=20)
        mc = ModelCheckpoint(filepath=checkpoint_path, monitor='val_accuracy', save_best_bonly=True, verbose=1, mode='max')
        
        # Train model
        model_fitted = model.fit(partial_train_data, partial_train_label,
                                 epochs=epochs, batch_size=batch_size, verbose=1, validation_data=(val_data, val_label),
                                callbacks=[es, mc])

        model_saved = load_model(checkpoint_path)

        # Evaluate model and store result
        val_err, val_mae = model_saved.evaluate(val_data, val_label, verbose=1)
        all_scores.append(val_mae)

        # Save model and plots
        model_path = os.path.join(folder_path, 'fitted_model_k{}.h5'.format(i + 1))
        model_saved.save(model_path, save_format='h5')
        plot_validation_info_kfold(model_fitted, i + 1, folder_path)

        # Save also weights and structure for backup
        weights_path = os.path.join(folder_path, 'weights_k{}.h5'.format(i + 1))
        model_saved.save_weights(weights_path)
        json_path = os.path.join(folder_path, 'structure_k{}.json'.format(i + 1))
        save_structure_in_json(model_saved, json_path)
     else:
        pass
...
def plot_validation_info_kfold(trained_model, i, save_folder):
    ...
    plt.figure()
    plt.plot(epochs, acc, color='orange', label='Training accuracy', linestyle='solid')
    plt.plot(epochs, val_acc, color='blue', label='Validation accuracy', linestyle='solid')
    ...
    plt.figure()
    plt.plot(epochs, loss, color='orange', label='Training loss', linestyle='solid')
    plt.plot(epochs, val_loss, color='blue', label='Validation loss', linestyle='solid')
