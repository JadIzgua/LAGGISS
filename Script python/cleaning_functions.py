...
def plot_histo_hsv(img_name):
    bgr = cv2.imread(img_name)
    hsv = cv2.cvtColor(bgr, cv2.COLOR_BGR2HSV)
    hist_full_v = cv2.calcHist([hsv], [2], None, [256], [0, 256])
    fig = plt.figure("Histogramme_{}".format(img_name), figsize=(10, 10))
    ...
    smooth_hist = savgol_filter(np.ravel(hist_full_v),31,3)

    plt.plot(hist_full_v)
    plt.plot(smooth_hist)
    extremas = argrelmax(smooth_hist, order=10)
    plt.plot(extremas[0], hist_full_v[extremas[0]], 'r+')

    plt.savefig("{}_images".format(str(os.path.basename(img_name))).replace(".", "_"), bbox_inches='tight')
    plt.close(fig)
